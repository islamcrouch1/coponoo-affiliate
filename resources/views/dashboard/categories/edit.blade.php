    ...
    <div class="row ">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{route('categories.update' , ['parent_cat' => request()->parent ,  'lang'=>app()->getLocale() , 'category'=>$category->id])}}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-10">
                                <input id="description_en" type="text" class="form-control @error('description_en') is-invalid @enderror" name="description_en" value="{{ $category->description_en }}"  autocomplete="description">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-10">
                                <select style="height: 50px;" class="form-control @error('parent') is-invalid @enderror" name="parent" value="{{ old('parent') }}">
                                    <option value="null" {{$category->parent == 'null' ? 'selected' : ''}}>{{__('Main Category')}}</option>

                                    @foreach ($categories as $category1)
                                    @if ($category1->id != $category->id)
                                    <option value="{{$category1->id}}" {{$category->parent == $category1->id ? 'selected' : ''}}>{{app()->getLocale() == 'ar' ? $category1->name_ar . ' - ' . $category1->country->name_ar : $category1->name_en . ' - ' . $category1->country->name_en}}</option>
                                    @endif
                                    @endforeach
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-10">
                                ...
                                <option value="{{ $country->id }}" {{$category->country->id == $country->id ? 'selected' : ''}} >{{ $country->name_en }}</option>
                                @endforeach
                                </select>
                                @error('country')
                                <span class="invalid-feedback" role="alert">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-10">
                                <input id="profit" type="double" class="form-control @error('profit') is-invalid @enderror" name="profit" value="{{ $category->profit }}"  autocomplete="profit" required>
