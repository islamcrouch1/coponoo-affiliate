          ...
          <form action="">
            <div class="row">
              ...
              <div class="col-md-4">
                ...
                <a href="{{route('shipping_rates.create', app()->getLocale()  )}}"> <button type="button" class="btn btn-primary">{{__('Create shipping rate')}}</button></a>

                @else
                <a href="#" aria-disabled="true"> <button type="button" class="btn btn-primary">{{__('Create shipping_rate')}}</button></a>
        ...
        <div class="card-body p-0">
          ...
          <table class="table table-striped projects">
              <thead>
                  ...
                  <?php if($shipping_rates !== Null){$shipping_rate = $shipping_rates[0];} ?>
                ...
                <td>
                  ...
                  @endif
                    <td class="project-actions">
                        ...
                        <a class="btn btn-info btn-sm" href="{{route('shipping_rates.edit' , ['lang'=>app()->getLocale() , 'shipping_rate'=>$shipping_rate->id])}}">
                      ...
                      @endif
                        ...
                        <a class="btn btn-info btn-sm" href="{{route('shipping_rates.restore' , ['lang'=>app()->getLocale() , 'shipping_rate'=>$shipping_rate->id])}}">
                          <i class="fas fa-pencil-alt">
                          </i>
                          {{__('Restore')}}
                      </a>
                    ...
                    @endif
                                ...
                                @if ((auth()->user()->hasPermission('shipping_rates-delete'))| (auth()->user()->hasPermission('shipping_rates-trash')))

                                    <form method="POST" action="{{route('shipping_rates.destroy' , ['lang'=>app()->getLocale() , 'shipping_rate'=>$shipping_rate->id])}}" enctype="multipart/form-data" style="display:inline-block">
                                        @csrf
                                        @method('DELETE')
          ...
          @else <h3 class="p-2">{{__('No shipping rates To Show')}}</h3>
          @endif
        </div>
        <!-- /.card-body -->
      </div>
