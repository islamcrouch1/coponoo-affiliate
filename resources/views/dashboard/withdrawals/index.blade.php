...
@section('adminContent')
    ...
    <section class="content">
          ...
          <form action="">
            <div class="row">
              <div class="col-md-2">
                ...
                <input type="text" name="search" placeholder="{{__('Search ...')}}" class="form-control" value="{{request()->search}}">
                </div>
              </div>
              ...
              <div class="col-md-2">
                <select class="form-control"  name="status" style="display:inline-block">
                  <option value="" selected>{{__('All Status')}}</option>
                    ...
                    <option value="recieved" {{request()->status == 'confirmrecieveded' ? 'selected' : ''}}>{{__('recieved')}}</option>
      ...
      <form method="POST" action="{{(route('withdrawals.export' , ['lang' => app()->getLocale() , 'from' => request()->from , 'to' => request()->to ]))}}" enctype="multipart/form-data">
        @csrf
        ...
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body p-0">
                      ...
                      <tbody>
                        @foreach ($withdrawals as $withdraw)
                          ...
                          <tr>
                            ...
                            <td style="">
                                <a href="{{route('users.show' , ['lang' => app()->getLocale() , 'user' => $withdraw->user->id])}}">
                                    {{$withdraw->user_name}}
                                </a>
                            </td>
                            <td style="">
                            ...
                            <td style="">{{$withdraw->amount}} {{' ' . $withdraw->user->country->currency}}</td>
                            <td style="">{{$withdraw->created_at}}</td>
                            <td style="">

                                @switch($withdraw->status)
                                @case('pending')
                                <span class="badge badge-primary badge-lg">{{__('Awaiting review from management')}}</span>
                                    @break
                                @case('recieved')
                                <span class="badge badge-info badge-lg">{{__('Your request has been received and is being reviewed for a deposit')}}</span>
                                    @break
                                @case('confirmed')
                                <span class="badge badge-success badge-lg">{{__('The amount has been deposited into your account')}}</span>
                                    @break
                                @case('canceled')
                                <span class="badge badge-danger badge-lg">{{__('Request rejected Please contact customer service to find out the reason')}}</span>
                                    @break
                                @default
                                @endswitch
                            ...
                            <td class="project-actions">
                                ...
                                <button type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#modal-primary-{{$withdraw->id}}">
                  ...
                  @else <h3 class="p-4">{{__('You have no recorded profit withdrawal requests')}}</h3>
                  @endif
                </div>
                <!-- /.card-body -->
              </div>
    ...
    <div class="modal fade" id="modal-primary-{{$withdraw->id}}">
        <div class="modal-dialog">
          <div class="modal-content bg-primary">
            ...
            <form method="POST" action="{{route('withdrawals.update.requset', ['lang'=> app()->getLocale() , 'withdrawal'=>$withdraw->id ])}}" enctype="multipart/form-data">
                @csrf
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-8">
                            ...
                            <select style="height: 50px;" class=" form-control @error('status') is-invalid @enderror" name="status" value="{{ old('status') }}" required autocomplete="status">
                                ...
                                <option value="pending" {{$withdraw->status == 'pending' ? 'selected' : ''}}>{{__('pending')}}</option>
                                <option value="recieved" {{$withdraw->status == 'recieved' ? 'selected' : ''}}>{{__('recieved')}}</option>
                                <option value="confirmed" {{$withdraw->status == 'confirmed' ? 'selected' : ''}}>{{__('confirmed')}}</option>
                                <option value="canceled" {{$withdraw->status == 'canceled' ? 'selected' : ''}}>{{__('canceled')}}</option>
                            </select>
                            @error('status')
