...
@section('adminContent')
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4">
                            ...
                            @if (auth()->user()->hasPermission('addresses-create'))
                                <a
                                    href="{{ route('addresses.create', ['lang' => app()->getLocale(), 'user' => $user->id]) }}">
                                    <button type="button" class="btn btn-primary">Create address</button></a>
                            @else
                            ...
                            @if (auth()->user()->hasPermission('addresses-read'))
                                <a
                                    href="{{ route('addresses.trashed', ['lang' => app()->getLocale(), 'user' => $user->id]) }}">
                                    <button type="button" class="btn btn-primary">addresses trash</button></a>
                            @else
                                <a href="#" aria-disabled="true"> <button type="button" class="btn btn-primary">addresses
                                        trash</button></a>
                            @endif
                        </div>
                    </div>
                                    ...
                                    <td class="project-actions">
                                        ...
                                        @if (!$address->trashed())
                                            @if (auth()->user()->hasPermission('addresses-update'))
                                                <a class="btn btn-info btn-sm"
                                                    href="{{ route('addresses.edit', ['lang' => app()->getLocale(), 'address' => $address->id, 'user' => $user->id]) }}">
                                        ...
                                        @else
                                            @if (auth()->user()->hasPermission('addresses-restore'))
                                                <a class="btn btn-info btn-sm"
                                                    href="{{ route('addresses.restore', ['lang' => app()->getLocale(), 'address' => $address->id, 'user' => $user]) }}">
                                                    <i class="fas fa-pencil-alt">
                                                    </i>
                                                    {{ __('Restore') }}
                                                </a>
    ...
    auth()->user()->hasPermission('addresses-trash'))
                                            <form method="POST"
                                                action="{{ route('addresses.destroy', ['lang' => app()->getLocale(), 'address' => $address->id, 'user' => $user->id]) }}"
            ...
            @else
                <h3 class="pl-2">No addresses To Show</h3>
                @endif
            </div>
            <!-- /.card-body -->
        </div>
