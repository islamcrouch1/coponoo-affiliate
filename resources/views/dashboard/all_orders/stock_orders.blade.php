                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                <a class="btn btn-info"
                                    href="{{ route('stock.orders', ['lang' => app()->getLocale(), 'status' => 'pending']) }}">{{ __('pending') . ' ( ' . \App\Aorder::where('status', 'pending')->count() . ' )' }}</a>
                                <a class="btn btn-info"
                                    href="{{ route('stock.orders', ['lang' => app()->getLocale(), 'status' => 'confirmed']) }}">{{ __('confirmed') . ' ( ' . \App\Aorder::where('status', 'confirmed')->count() . ' )' }}</a>
                                <a class="btn btn-info"
                                    href="{{ route('stock.orders', ['lang' => app()->getLocale(), 'status' => 'rejected']) }}">{{ __('rejected') . ' ( ' . \App\Aorder::where('status', 'rejected')->count() . ' )' }}</a>
                                <a class="btn btn-info"
                                    href="{{ route('stock.orders', ['lang' => app()->getLocale(), 'status' => 'canceled']) }}">{{ __('canceled') . ' ( ' . \App\Aorder::where('status', 'canceled')->count() . ' )' }}</a>
                            </div>
                                ...
                                <tbody>
                                    @foreach ($orders as $order)
                                        <tr>
                                            ...
                                            <td>
                                                <a
                                                    href="{{ route('users.show', [app()->getLocale(), $order->user_id]) }}">
                                                    <small>
                                                        {{ $order->user_name }}
                                                    </small>
                                                </a>
        ...
        <div class="modal fade" id="modal-primary-{{ $order->id }}">
            <div class="modal-dialog">
                <div class="modal-content bg-primary">
                    ...
                    <form method="POST"
                        action="{{ route('stock.orders.change', ['lang' => app()->getLocale(), 'order' => $order->id]) }}"
